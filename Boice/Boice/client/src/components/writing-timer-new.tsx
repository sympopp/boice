...
const WritingTimerCore = () => {
  ...
  const isStageAvailable = (stage: string): boolean => {
    ...
    // In initial or front half, all stages with time remaining are available
  ...
  // Check if all daily time is used up
  const isDailyTimeUsed = (): boolean => {
    return dailyTimeLeft <= 0;
  };
  ...
  useEffect(() => {
    try {
      ...
      const savedTomorrowPlan = localStorage.getItem('tomorrowPlan');
      
      if (savedTomorrowPlan) {
        setTomorrowPlan(savedTomorrowPlan);
      }
      ...
      if (savedData) {
        ...
        if (data.lastDate !== today) {
          ...
          // Use tomorrow's plan as today's plan if available
          if (savedTomorrowPlan) {
            setDailyPlan(savedTomorrowPlan);
            // Clear tomorrow's plan since it's now today's plan
          ...
          setShowPlanDialog(!data.dailyPlan && dailyTimeLeft === FOUR_HOURS_IN_SECONDS);
          
          // If all daily time is used up, prompt for tomorrow's plan
          if (data.dailyTimeLeft <= 0 && !savedTomorrowPlan) {
            setShowTomorrowPlanDialog(true);
          }
        }
      } else {
  ...
  useEffect(() => {
    try {
      ...
      // Check if all daily time is used up to prompt for tomorrow's plan
      if (dailyTimeLeft <= 0 && !localStorage.getItem('tomorrowPlan')) {
        setShowTomorrowPlanDialog(true);
      }
    } catch (error) {
  ...
  // Update weekly time budget and track total time
  const updateWeeklyTimeLeft = (type: string, usedSeconds: number) => {
    // Update the stage time left
    setWeeklyTimeLeft(prev => ({
      ...prev,
      [type]: Math.max(0, prev[type as keyof typeof prev] - usedSeconds)
    }));
    
    // Update total seconds worked
    setTotalSecondsWorked(prev => prev + usedSeconds);
    
    // Update week phase
    const newPhase = determineWeekPhase(
      totalSecondsWorked + usedSeconds,
      currentDay,
      {
        ...weeklyTimeLeft,
        [type]: Math.max(0, weeklyTimeLeft[type as keyof typeof weeklyTimeLeft] - usedSeconds)
      }
    );
    setWeekPhase(newPhase);
  };
  ...
  function completeSession() {
    if (sessionTime > 0) {
      ...
      // Update weekly time budget and tracking
  ...
  function calculateTotalWeeklyTime() {
    const total = Object.values(weeklyTimeLeft).reduce((sum, current) => {
      if (typeof current === 'number') {
        return sum + (STAGE_MAX_SECONDS - current);
      }
      return sum;
    }, 0);
            ...
            <>
              ...
              <div className="flex">
                <Button 
                  className="flex-1"
                  onClick={handleStart}
                  disabled={!sessionType || !projectTitle.trim() || availableStages.length === 0}
                >
                  Start Session
                </Button>
              </div>
            ...
            <div className="space-y-4">
              {dailyTimeLeft <= 0 ? (
                <div className="text-center p-6 bg-gray-100 rounded-lg">
                  <h3 className="font-medium text-lg mb-2">Daily Writing Complete</h3>
                  <p className="text-gray-600 mb-4">You've used all your writing time for today.</p>
                  <Button 
                    variant="outline"
                ...
                <div className="text-center p-6 bg-gray-100 rounded-lg">
                  <h3 className="font-medium text-lg mb-2">Weekly Stage Limits Reached</h3>
                  <p className="text-gray-600">You've used your allotted time for all writing stages this week.</p>
                </div>
              )}
            </div>
          )}
